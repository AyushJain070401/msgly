---
'@msgly/plivo-voice': minor
'@msgly/vonage-voice': minor
'@msgly/core': minor
---

Add Plivo Voice and Vonage Voice, taking voice from one provider to three.
...
cannot come from the hub's `on('message')` handler, which runs *after* the
...
application JWT — an Application ID plus a private key, *not* the
...
no-answer are the *person*, not the line. They may answer next time, and
