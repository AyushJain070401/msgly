...
export function createGenesysVoiceAdapter(config: GenesysVoiceConfig): GenesysVoiceAdapter {
   ...
   * because the OAuth client's role lacks `telephony:did:view`, which is a
  ...
  > {
    try {
      const headers = await authHeaders();
      const res = await fetch(
        `${apiBase}/api/v2/telephony/providers/edges/dids?phoneNumber=${encodeURIComponent(config.phoneNumber)}&pageSize=100`,
        { headers },
      );
  ...
  async function verifyPhoneNumber(): Promise<PhoneNumberCheckResult> {
    ...
    if (problem) {
      return {
        ...
        hint: `GenesysVoiceConfig.phoneNumber ${problem}.`,
      };
    }
    ...
    if (ownership.owned === false) {
      return {
        ...
        status: 'not_owned',
        phoneNumber: config.phoneNumber,
        hint: `${config.phoneNumber} is not a number on this Genesys Cloud account. Check it under Admin → Telephony → DID Numbers.`,
      };
    }
    return {
      ok: true,
  ...
  async function verifyCredentials(): Promise<CredentialsCheckResult> {
    ...
    if (phoneProblem) {
      return {
        ok: false,
        reason: 'unauthorized',
        hint: `GenesysVoiceConfig.phoneNumber ${phoneProblem}.`,
      };
    }
