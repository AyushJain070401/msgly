...
Messenger's webhook carries only the sender's PSID — no name, no photo. The
...
Every inbound message then arrives with `contact.displayName` and
`contact.avatarUrl` filled in. Messenger has no handle, so `contact.username`
stays unset — that field is Instagram-only.
...
```typescript
const profile = await messenger.getSenderProfile(message.contact.channelUserId);
// → { name: 'Ayush Jain', avatarUrl: 'https://…' }
```
